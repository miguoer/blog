...
CSS 不会影响 DOM 渲染，但是影响 DOM 解析。在 Chrome 调试工具中设置限速，可以看到效果。
...
```
JS会等待CSS回来，不然会引起页面抖动。

### CSS会影响Dom content loaded吗
...
影不影响取决于css下面还有没有脚本。如果css下面有脚本，那么就会阻塞，否则不会阻塞。为什么有脚本就会阻塞。这是因为脚本可以操作DOM，加载CSS的时候不知道下面的脚本会不会影响DOM，所以就要一直等着。