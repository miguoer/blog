...
数据同构的好处是可以加快首屏渲染速度，较少首屏网络请求。尤其是像我们现在这个项目，首页90%的内容都是需要经过网络请求后再加载的，这个时候做数据同构会极大提升用户体验。

数据同构主要用到的技术是redux。实现整套redux的过程步骤多，难度不大。最后贴一下最终服务端同构的完整代码，以及客户端入口代码，redux剩下的代码可以按官网操作。
